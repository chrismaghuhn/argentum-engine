...
class StatsModifiedEventHistoryClosureTest : FunSpec({
    val self = EntityId("p1")
    val opponent = EntityId("p2")
    val target = EntityId("stats-target-runtime-id")
    val wrongTarget = EntityId("wrong-target-runtime-id")
    val targetName = "Hidden Target Name"
    val sourceName = "Hidden Source Name"
    ...
    fun state() = GameState(
        ...
        objectIdentityStamps = mapOf(target to 7L, wrongTarget to 8L),
    )

    fun event() = StatsModifiedEvent(
        targetId = target,
    ...
    fun automaticProjection(perspectivePlayerId: EntityId) =
        CommittedPerspectiveEventSource(CardRegistry()).also { source ->
            ...
            semanticEpisodeId = "stats-modified-history-closure",
            perspectivePlayerId = perspectivePlayerId,
    ...
    test("StatsModifiedEvent projects stable scalar facts for both perspectives") {
        ...
        projections.forEach { projection ->
            ...
            projection.classifications.single().rawEventType shouldBe "StatsModifiedEvent"
            ...
            projection.batch.entries.single().eventFamily shouldBe PerspectiveEventFamily.STATS_MODIFIED
            projection.batch.entries.single().semanticPayload shouldBe buildJsonObject {
                put("type", "stats_modified")
                put("powerChange", 2)
                put("toughnessChange", -3)
            }
    ...
    test("StatsModifiedEvent binds its target through the after-object witness") {
        ...
        results.forEach { result ->
            ...
            candidate.afterWitness shouldBe HistoryCObjectWitness(target, 7L)
    ...
    test("StatsModifiedEvent raw authority rejects malformed target bindings") {
        ...
        validate(
            candidate.copy(afterWitness = HistoryCObjectWitness(wrongTarget, 8L)),
