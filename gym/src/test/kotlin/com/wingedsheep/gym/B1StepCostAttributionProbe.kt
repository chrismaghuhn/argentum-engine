...
/**
 * Test-only coarse exclusive timing/allocation probe. It is active only for the explicit
 * attribution run and records large method boundaries, never individual candidates or entities.
 */
internal object B1StepCostAttributionProbe {
    ...
    internal data class SegmentSnapshot(
        ...
        val policyWallNanos: Long,
        val policyAllocatedBytes: Long?,
        val policyAllocationSamples: Long,
    ...
    internal class Session internal constructor() {
        ...
        internal fun beginSegment(label: String): SegmentHandle {
            ...
            segments += segment
            if (!currentSegment.compareAndSet(null, segment)) markIntegrityError()
            return SegmentHandle(this, segment)
        }

        internal fun endSegment(segment: Segment) {
            if (!currentSegment.compareAndSet(segment, null)) markIntegrityError()
        }
        ...
        internal fun beginPolicy(): PolicyHandle? {
            val segment = currentSegment.get() ?: return null
            return PolicyHandle(
                session = this,
                segment = segment,
        ...
        internal fun endPolicy(handle: PolicyHandle) {
            val wall = (System.nanoTime() - handle.startNanos).coerceAtLeast(0L)
            val allocated = handle.startAllocatedBytes?.let { start ->
                currentThreadAllocatedBytes()?.minus(start)?.coerceAtLeast(0L)
            }
            handle.segment.recordPolicy(wall, allocated)
        }
        ...
        internal fun endPhase(family: String) {
            ...
            state.segment.phase(family).record(exclusiveWall, exclusiveAllocated)
        ...
        internal fun snapshot(): Snapshot {
            if (currentSegment.get() != null || threadState.get() != null) markIntegrityError()
            return Snapshot(
                schemaVersion = "argentum-b1-step-cost-attribution-v2",
    ...
    internal class PolicyHandle internal constructor(
        private val session: Session,
        internal val segment: Segment,
        internal val startNanos: Long,
        internal val startAllocatedBytes: Long?,
    ) : AutoCloseable {
        private val closed = AtomicBoolean()

        override fun close() {
            if (closed.compareAndSet(false, true)) session.endPolicy(this)
        }
    }
    ...
    internal class Segment(private val label: String) {
        ...
        private val policyWallNanos = LongAdder()
        private val policyAllocatedBytes = LongAdder()
        private val policyAllocationSamples = LongAdder()
        private val phases = ConcurrentHashMap<String, PhaseTotals>()

        fun recordPolicy(wallNanos: Long, allocatedBytes: Long?) {
            policyWallNanos.add(wallNanos)
            if (allocatedBytes != null) {
                policyAllocatedBytes.add(allocatedBytes)
                policyAllocationSamples.increment()
            }
        }

        fun phase(family: String): PhaseTotals = phases.computeIfAbsent(family) { PhaseTotals() }

        fun snapshot(): SegmentSnapshot {
            val policySamples = policyAllocationSamples.sum()
            return SegmentSnapshot(
                label = label,
                transitions = transitions.sum(),
                transitionWallNanos = transitionWallNanos.sum(),
                policyWallNanos = policyWallNanos.sum(),
                policyAllocatedBytes = policyAllocatedBytes.sum().takeIf { policySamples > 0L },
                policyAllocationSamples = policySamples,
    ...
    internal class PhaseTotals {
        ...
        fun record(wall: Long, allocated: Long?) {
            wallNanos.add(wall)
            if (allocated != null) {
    ...
    internal fun beginPolicy(): PolicyHandle? = activeSession.get()?.beginPolicy()
