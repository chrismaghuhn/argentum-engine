...
/** Damnation (PLC #85): destroy all creatures; they can't be regenerated. */
class DamnationScenarioTest : ScenarioTestBase() {

    private fun game() = scenario()
        .withPlayers("Player1", "Player2")
        .withCardInHand(1, "Damnation")
    ...
    init {
        test("destroys every creature but leaves noncreature permanents") {
            val game = game()
            val result = game.castSpell(1, "Damnation")

            withClue("Damnation should cast: ${result.error}") {
        ...
        test("ignores regeneration shields") {
            ...
            val result = game.castSpell(1, "Damnation")
            withClue("Damnation should cast against a shielded creature: ${result.error}") {
                result.error shouldBe null
            }
            game.resolveStack()
