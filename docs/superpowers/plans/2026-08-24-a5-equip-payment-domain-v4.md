...
- [ ] Step 3: Implement isSupportedEquipPayment. It must require the same actor-perspective mapping used by normal ObservationBuilder target publication: ActionTargetDomainMapper.map(legalAction) { entityId -> visibility.isEntityReferenceAddressableTo(state = state, entityId = entityId, viewingPlayerId = action.playerId) } (or a shared private mapping helper) must return Supported; one canonical mandatory fixed target requirement with minTargets 1 and maxTargets 1 and at least one candidate; one corresponding Rules ability target requirement; an ordinary mana-only base cost; no target-dependent generic reduction; and a public cost parsed using only colored, colorless, and generic symbols.
...
- [ ] Step 5: Submit an entity that is not contained in the published target-domain candidates and assert rejection with unchanged environment step count. Separately submit an incomplete or incorrect {1} ExplicitV2 allocation and assert rejection with unchanged environment step count. As a positive control, when the published domain contains multiple legal candidates, submit another published candidate and assert it remains valid; never reject a valid alternate public target.
