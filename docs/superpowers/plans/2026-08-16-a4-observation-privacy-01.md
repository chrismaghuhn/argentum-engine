...
engine-action/decision payload for fields such as provenance-specific ability
ordinal/structure and cast choices; generated ability handles, donor EntityIds,
transport IDs, and presentation text remain outside the digest.
...
Add the deterministic structured engine-action/decision payload to the fingerprint so fields such as provenance-specific ability ordinal/structure, `castFaceDown`, `faceIndex`, modes, payments, and repeat counts remain distinguishable. Resolve printed, granted, static, emblem, class-level, intrinsic, and donor-derived abilities through their authoritative sources; never add description, a donor EntityId, or a JVM-generated ability handle as a semantic key. If two actions remain distinguishable only by prose, fail the focused test and record that as an explicit structured contract gap.
