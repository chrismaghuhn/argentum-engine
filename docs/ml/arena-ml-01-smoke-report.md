...
Date: 2026-09-16 (evidence corrected per exact-SHA review)
Status:

```text
EXECUTION_PLUMBING_PASS = YES
REAL_CUDA_CHARACTERIZATION_PASS = YES
TERMINAL_GAME = NO
ZERO_UNSUPPORTED = NO
COMPLETE_GAME_SMOKE = BLOCKED
```

The fail-closed truncation is a successful characterization of a trusted-path boundary — not a
completed model-vs-engine game and never reported as one. Review verdicts (P1/P2/P3,
CODE_REVIEW_PASS, FINAL_ACCEPTANCE_PASS) are assigned only by independent review and are not
generated by this artifact.

## Verdict

The accepted C1_06 model, through the accepted C1_07A/B/C live-policy stack, successfully and
genuinely controlled the Akiri seat through its mulligan decision and 69 ordinary gameplay
decisions against the Engine-AI Chevill seat on real CUDA inference, with zero fallback, zero
worker failures, zero stale rejections, and zero CPU inference. At decision #71 the trusted path
failed closed on a real decision boundary and the smoke was truncated by design. That is the
correct outcome under the task contract: no heuristic, no fallback, no hidden substitution was
ever used. The deterministic run kept on its first-pull hand, so London bottoming was never
exercised and is reported as NOT_EXERCISED rather than inferred.
...
PREGAME_ML_ACTION_TYPES=KeepHand
MULLIGAN_MODEL_CONTROLLED=YES
BOTTOMING_MODEL_CONTROLLED=NOT_EXERCISED
...
- Signal: trusted observation diagnostic `PAYMENT_DOMAIN_UNSUPPORTED`
  ...
  deterministic decision #71 state, Shadowspear's {1} activation was enumerated as a legal,
  affordable candidate, but the trusted observation path did not publish a complete
  `PaymentDomainV5` and emitted the diagnostic, so `LivePolicySourceAdapter.fromObservationResult`
  failed closed with `PolicySeatFailureCode.UNSUPPORTED_STRUCTURED_DECISION` before any model
  request was issued.
- Why this is correct: the live-policy stack forbids publishing a partial payment domain or any
  heuristic substitute for the ML seat. The engine, the gym observation layer, and the game-server
  policy stack each held their own fail-closed contract; the composition surfaces as one
  deterministic, trustworthy boundary.
- Disposition: `PAYMENT_LOWER_LEVEL_ROOT_CAUSE = UNCHARACTERIZED`. This report does not claim why
  the domain was refused beyond the boundary above — in particular it makes no claim about which
  `paymentDomainV5For(...)` step returned null and no generic claim that PaymentDomainV5 does not
  represent paid ability activations. A separate follow-up must first create a minimal RED
  characterization isolating exactly where `paymentDomainV5For(...)` returns null; only then is a
...
green in this run. The PolicyTieRng cursor never advanced on the rejected decision.
